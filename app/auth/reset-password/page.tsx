...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    ...
    console.log('reset-password: code=', code ? 'PRESENTE' : 'ASSENTE')
    ...
    // exchangeCodeForSession funziona client-side perché
    // il PKCE verifier è in localStorage (stesso dominio)
      ...
      <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: '32px 28px', border: '1px solid rgba(255,255,255,0.08)' }}>
        {success ? (
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Reindirizzamento al login…</p>
          </div>
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#0c1a2a,#0e2a3a)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 20px', fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
        <Suspense fallback={
          <div style={{ width: 36, height: 36, border: '3px solid rgba(14,136,165,0.2)', borderTopColor: '#0e88a5', borderRadius: '50%', animation: 'spin .8s linear infinite' }} />
        }>
          <ResetPasswordForm />
        </Suspense>
      </div>
